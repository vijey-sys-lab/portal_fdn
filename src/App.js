  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg-primary)",padding:24 }}>
      <div style={{ background:"var(--bg-secondary)",border:"1px solid var(--border-light)",borderRadius:20,padding:"48px 40px",maxWidth:440,width:"100%",textAlign:"center" }}>
        ...
        <h2 style={{ fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,marginBottom:10,color:"var(--text-primary)" }}>Awaiting Approval</h2>
        <p style={{ fontSize:14,color:"var(--text-secondary)",lineHeight:1.8,marginBottom:24 }}>
          ...
          Your account has been created. Please wait for your admin to approve your profile before you can access the portal.
        </p>
        <div style={{ background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:10,padding:16,marginBottom:24,textAlign:"left" }}>
          <div style={{ display:"flex",justifyContent:"space-between",marginBottom:8 }}><span style={{ fontSize:12,color:"var(--text-muted)" }}>NIN</span><span className="badge badge-accent">{userData?.nin}</span></div>
          <div style={{ display:"flex",justifyContent:"space-between",marginBottom:8 }}><span style={{ fontSize:12,color:"var(--text-muted)" }}>Team</span><span className="badge badge-purple">{userData?.team}</span></div>
          <div style={{ display:"flex",justifyContent:"space-between" }}><span style={{ fontSize:12,color:"var(--text-muted)" }}>Status</span><span className="badge badge-warning">Pending Approval</span></div>
        </div>
        <button onClick={logout} style={{ background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,padding:"10px 24px",color:"#ef4444",fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer",fontSize:14 }}>Sign Out</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg-primary)" }}>
      <div style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",borderRadius:16,padding:"40px 32px",maxWidth:400,textAlign:"center" }}>
        ...
        <p style={{ color:"var(--text-secondary)",fontSize:14,marginBottom:20 }}>Your internship has been cancelled. Contact your admin for more details.</p>
        <button onClick={logout} style={{ background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,padding:"10px 24px",color:"#ef4444",fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer",fontSize:14 }}>Sign Out</button>
      </div>
    </div>
  );
...
function ProtectedRoute({ children, allowedRoles }) {
  ...
  if (userRole === "cancelled") return <CancelledScreen />;
  ...
  return (
    <ErrorBoundary>
      <BrowserRouter>
        <AuthProvider>
          <AppRoutes />
          <Toaster position="top-right" toastOptions={{ style:{ background:"var(--bg-card)",color:"var(--text-primary)",border:"1px solid var(--border-light)",fontFamily:"'DM Sans',sans-serif",fontSize:13 }, success:{ iconTheme:{ primary:"#22c55e",secondary:"#000" } }, error:{ iconTheme:{ primary:"#ef4444",secondary:"#fff" } } }} />
        </AuthProvider>
      </BrowserRouter>
    </ErrorBoundary>
  );
