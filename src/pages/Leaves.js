...
export default function Leaves() {
  ...
  const [allLeaves, setAllLeaves] = useState([]); // always full list for counts
  ...
  async function fetchLeaves() {
    setLoading(true);
    try {
      // Always fetch ALL leaves first for correct counts
      const q = query(collection(db, "leaves"), orderBy("createdAt", "desc"));
      let data = (await getDocs(q)).docs.map(d => ({ id: d.id, ...d.data() }));

      // Admin only sees their interns' leaves
      if (userRole === "admin" && userData?.uid) {
        const internsSnap = await getDocs(
          query(collection(db, "interns"), where("assignedAdmin", "==", userData.uid))
        );
        const myUids = internsSnap.docs.map(d => d.data().uid);
        data = data.filter(l => myUids.includes(l.internId));
      }

      setAllLeaves(data); // full list for tab counts
      setLeaves(data);    // filtered list for display
    } catch (e) {
      console.error(e);
  ...
  const typeColors = {
    sick: "danger", personal: "info", emergency: "warning", other: "purple"
  };

  // Filter for display only — counts always use allLeaves
  const displayed = allLeaves.filter(l => {
    const matchSearch =
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
        <div className="tabs" style={{ margin: 0 }}>
          {tabs.map(t => (
            ...
            >
              {t.label}
              <span style={{ opacity: 0.7, marginLeft: 4 }}>({counts[t.key]})</span>
            </button>
          ))}
        </div>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 16, marginBottom: 10, flexWrap: "wrap", alignItems: "center" }}>
                  <span style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                    📅 <strong>{leave.fromDate}</strong> → <strong>{leave.toDate}</strong>
                  </span>
                  <span className="badge badge-accent">
                    {getDays(leave.fromDate, leave.toDate)} day(s)
                  </span>
                </div>
                ...
                {leave.adminRemark && (
                  <div style={{
                    marginTop: 10, background: "var(--bg-primary)", borderRadius: 8,
                    padding: "10px 14px", fontSize: 13, color: "var(--text-secondary)",
                    borderLeft: `3px solid var(--${leave.status === "approved" ? "success" : "danger"})`
                  }}>
              ...
              {leave.status === "pending" && (
                <button
                  ...
                  onClick={() => { setRemarkModal(leave); setRemark(""); setDecision("approved"); }}
                >
                  Review
                </button>
              )}
            ...
            <div className="modal-body">
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
                  {remarkModal.type} leave · {remarkModal.fromDate} → {remarkModal.toDate} ({getDays(remarkModal.fromDate, remarkModal.toDate)} days)
                </div>
            ...
            <div className="modal-footer">
              ...
              >
                {saving
                  ...
                  : `${decision === "approved" ? "Approve" : "Reject"} Leave`
                }
              </button>
            </div>
          </div>
