      ...
      const results = await Promise.allSettled([
        ...
        userData.team
          ? getDocs(query(
              ...
              where("assignType", "==", "team")
            ))
          : Promise.resolve({ docs: [] }),
        getDocs(query(
          collection(db, "tasks"),
          where("team", "==", "All Interns"),
          where("assignType", "==", "team")
        )),
      ]);
  ...
  return (
    <Layout title="My Tasks" subtitle="Tasks assigned to you">
      <div className="page-header">
        <div>
          <h2>My Tasks</h2>
          <p>Tasks assigned to you by your admin</p>
        </div>
        <div style={{
          background: "var(--bg-card)", border: "1px solid var(--border)",
          borderRadius: 8, padding: "8px 16px", fontSize: 13, color: "var(--text-secondary)"
        }}>
      ...
      <div style={{
        ...
        💡 Once you complete a task, go to{" "}
        <a href="/journal" style={{ color: "var(--accent)", fontWeight: 700, textDecoration: "none" }}>
          Daily Journal
        </a>
        {" "}and submit your work with a Google Drive link and what you learned.
      </div>
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card">
          <div className="stat-icon" style={{ background: "rgba(245,166,35,0.1)", color: "var(--accent)" }}>
            📋
          </div>
          <div className="stat-content">
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
                    ...
                    {/* Always show Assigned — never show old completed/status from Firestore */}
                    <span className="badge badge-accent">Assigned</span>
                    {task.assignType === "team" && (
                      <span className="badge badge-purple" style={{ fontSize: 10 }}>Team Task</span>
                    )}
                    {task.assignType === "specific" && (
                      <span className="badge badge-info" style={{ fontSize: 10 }}>For You</span>
                    )}
                    {isOverdue && (
                  ...
                  <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                      Team: <span style={{ color: "var(--text-secondary)" }}>{task.team}</span>
                    </span>
                    {task.deadline ? (
                      <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                        Deadline:{" "}
