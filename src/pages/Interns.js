...
import { MdAdd, MdClose, MdSearch, MdDownload, MdBlock } from "react-icons/md";
...
function generateOfferLetter(intern) {
  ...
  pdf.text("To,", 20, 67);
  pdf.setFontSize(12); pdf.text(intern.name, 20, 74);
  ...
  const intro = "We are pleased to offer you an internship position at BOSS Foundation. After reviewing your profile, we confirm your selection for the internship program.";
  ...
  y += 4;
  pdf.setFontSize(10); pdf.text("We look forward to your valuable contribution and wish you a rewarding internship experience.", 20, y);
...
export default function Interns() {
  ...
  async function handleAdd() {
    if (!form.name || !form.email || !form.password || !form.team || !form.startDate || !form.endDate) { toast.error("Fill all required fields"); return; }
  ...
  async function handleCancel(intern) {
    if (!window.confirm(`Cancel internship for ${intern.name}?`)) return;
    try { await updateDoc(doc(db, "interns", intern.id), { status: "cancelled" }); toast.success("Cancelled"); fetchInterns(); }
    catch { toast.error("Error"); }
  }

  const filtered = interns.filter(i => {
    const matchSearch = i.name?.toLowerCase().includes(search.toLowerCase()) || i.email?.toLowerCase().includes(search.toLowerCase()) || i.nin?.toLowerCase().includes(search.toLowerCase());
    return activeTab === "all" ? matchSearch : matchSearch && i.status === activeTab;
  });

  const tabs = [{ key: "all", label: "All" }, { key: "active", label: "Active" }, { key: "cancelled", label: "Cancelled" }];

  return (
    <Layout title="Interns" subtitle="Manage intern accounts and tenures">
      <div className="page-header">
        <div><h2>Intern Management</h2><p>Add interns — accounts are active immediately</p></div>
      ...
      <div className="card">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
          ...
          <div className="search-bar"><MdSearch /><input placeholder="Search interns..." value={search} onChange={e => setSearch(e.target.value)} /></div>
        </div>
        ...
        : filtered.length === 0 ? (
          ...
          <div className="table-wrapper">
            <table>
              <thead><tr><th>Intern</th><th>NIN</th><th>Team</th><th>College</th><th>Start</th><th>End</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {filtered.map(intern => (
                  <tr key={intern.id}>
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        {intern.photoURL
                          ...
                          : <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--accent-glow)", border: "2px solid var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12, color: "var(--accent)" }}>{intern.name?.charAt(0).toUpperCase()}</div>
                        }
                        <div>
                    ...
                    <td><span className={`badge badge-${intern.status === "active" ? "success" : "danger"}`}>{intern.status}</span></td>
                    <td>
                      <div style={{ display: "flex", gap: 6 }}>
                        {intern.status === "active" && <button className="btn btn-danger btn-sm" onClick={() => handleCancel(intern)}><MdBlock /> Cancel</button>}
                        <button className="btn btn-secondary btn-sm" onClick={() => generateOfferLetter(intern)} title="Offer Letter"><MdDownload /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="modal modal-lg">
            ...
            <div className="modal-body">
              <div className="form-grid">
                <div className="form-group"><label className="form-label">Full Name *</label><input className="form-input" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Intern Name" /></div>
                <div className="form-group"><label className="form-label">Email *</label><input className="form-input" type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="intern@email.com" /></div>
                ...
                <div className="form-group"><label className="form-label">College</label><input className="form-input" value={form.college} onChange={e => setForm({...form, college: e.target.value})} placeholder="College name" /></div>
