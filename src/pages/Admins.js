...
import { MdAdd, MdCheck, MdClose, MdSearch, MdPerson } from "react-icons/md";
...
export default function Admins() {
  ...
  async function fetchAdmins() {
    ...
    } catch (e) { toast.error("Error loading admins"); }
    setLoading(false);
  }
  ...
  async function handleAdd() {
    ...
    try {
      ...
      toast.error(msgs[e.code] || e.message || "Error adding admin");
    }
    setSaving(false);
  }
  ...
  async function handleApprove(admin) {
    try {
      ...
      toast.success("Admin approved"); fetchAdmins();
  ...
  return (
    <Layout title="Admins" subtitle="Manage admin accounts and approvals">
      <div className="page-header">
        <div><h2>Admin Management</h2><p>Add and manage admins for each team</p></div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <span style={{ fontSize: 13, color: "var(--text-secondary)" }}>{filtered.length} admin(s)</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3>No Admins Found</h3><p>Add your first admin to get started</p>
          </div>
        ) : (
          <div className="table-wrapper">
            <table>
              <thead>
                <tr><th>Name</th><th>Email</th><th>Team</th><th>Start Date</th><th>End Date</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {filtered.map(admin => (
                  <tr key={admin.id}>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 6 }}>
                        {admin.status === "pending" && (
                          <button className="btn btn-success btn-sm" onClick={() => handleApprove(admin)}><MdCheck /> Approve</button>
                        )}
                        {admin.status === "approved" && (
                          <button className="btn btn-danger btn-sm" onClick={() => handleTerminate(admin)}><MdClose /> Terminate</button>
                        )}
                      </div>
                    </td>
                  </tr>
