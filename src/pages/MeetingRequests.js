...
export default function MeetingRequests() {
  ...
  async function handleAction() {
    if (actionForm.response === "approve" && (!actionForm.meetLink || !actionForm.approvedTime)) {
      toast.error("Provide meet link and time"); return;
    }
    setSaving(true);
    try {
      await updateDoc(doc(db, "meetingRequests", actionModal.id), {
        status: actionForm.response === "approve" ? "approved" : "rejected",
        meetLink: actionForm.meetLink,
        approvedTime: actionForm.approvedTime,
        respondedBy: userData?.name,
  ...
  return (
    <Layout title="Meeting Requests" subtitle="Admin requests for meetings with you">
      <div className="page-header">
        <div><h2>Meeting Requests</h2><p>Review and respond to admin meeting requests</p></div>
        {pending.length > 0 && (
          <div style={{ background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 8, padding: "8px 14px", fontSize: 13, color: "var(--warning)", fontWeight: 600 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {req.status === "approved" && (
                      <div style={{ marginTop: 8 }}>
                        <div style={{ fontSize: 12, color: "var(--success)" }}>Approved time: {req.approvedTime}</div>
                        {req.meetLink && <a href={req.meetLink} target="_blank" rel="noreferrer" className="dj-link" style={{ fontSize: 12, marginTop: 4 }}>Join Meet</a>}
                      </div>
                    )}
                  </div>
                  {req.status === "pending" && (
                    <button className="btn btn-primary btn-sm" onClick={() => { setActionModal(req); setActionForm({ meetLink: "", approvedTime: req.preferredTime, response: "approve" }); }}>
                      Respond
                    </button>
                  )}
                </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setActionModal(null)}>
          <div className="modal">
            <div className="modal-header">
              <span className="modal-title">Respond to Meeting Request</span>
              <button className="modal-close" onClick={() => setActionModal(null)}><MdClose /></button>
            </div>
              ...
              {actionForm.response === "approve" && (
                <>
                  <div className="form-group">
                    ...
                    <input className="form-input" type="datetime-local" value={actionForm.approvedTime} onChange={e => setActionForm({...actionForm, approvedTime: e.target.value})} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Google Meet Link *</label>
                    <input className="form-input" value={actionForm.meetLink} onChange={e => setActionForm({...actionForm, meetLink: e.target.value})} placeholder="https://meet.google.com/..." />
                  </div>
                </>
              )}
            </div>
            <div className="modal-footer">
              ...
              <button className={`btn ${actionForm.response === "approve" ? "btn-success" : "btn-danger"}`} onClick={handleAction} disabled={saving}>
                {saving ? "Saving..." : actionForm.response === "approve" ? "Approve & Send Link" : "Reject Request"}
              </button>
            </div>
          </div>
        </div>
