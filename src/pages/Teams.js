...
export default function Teams() {
  ...
  async function fetchTeams() {
    try {
      ...
      if (userRole === "superadmin") {
        ...
        q = query(collection(db, "teams"), where("domain", "==", userData.team));
      }
  ...
  async function handleCreate() {
    ...
    try {
      await addDoc(collection(db, "teams"), {
        ...
        // If admin creates, needs SA approval. If SA creates, auto-approved.
      ...
      setShowModal(false);
      setForm({ name: "", domain: "HR", description: "" });
      fetchTeams();
  ...
  async function handleApprove(team) {
    try {
      await updateDoc(doc(db, "teams", team.id), { status: "approved" });
      toast.success("Team approved!"); fetchTeams();
    } catch { toast.error("Error"); }
  }

  async function handleReject(team) {
    try {
      await updateDoc(doc(db, "teams", team.id), { status: "rejected" });
      toast.success("Team rejected."); fetchTeams();
    } catch { toast.error("Error"); }
  }

  const grouped = DOMAINS.reduce((acc, domain) => {
    acc[domain] = teams.filter(t => t.domain === domain);
    return acc;
  }, {});
    ...
    <Layout title="Teams" subtitle="Manage sub-teams under each domain">
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
          <MdAdd /> {userRole === "superadmin" ? "Create Team" : "Request Team"}
        </button>
      </div>
      ...
      {loading ? <div className="loading-center"><div className="spinner" /></div> : (
        DOMAINS.map(domain => {
          ...
          if (domainTeams.length === 0 && userRole !== "superadmin") return null;
          return (
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            <div className="modal-header">
              <span className="modal-title">{userRole === "superadmin" ? "Create New Team" : "Request New Team"}</span>
              <button className="modal-close" onClick={() => setShowModal(false)}><MdClose /></button>
            </div>
            <div className="modal-body">
              <div className="form-group">
                <label className="form-label">Team Name *</label>
                <input className="form-input" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g., Frontend Team A" />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <textarea className="form-textarea" style={{ minHeight: 80 }} value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="What does this team focus on?" />
              </div>
              {userRole !== "superadmin" && (
                <div className="alert alert-info">Team name will be reviewed and approved by Super Admin before activation.</div>
              )}
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => setShowModal(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleCreate} disabled={saving}>
                {saving ? "Saving..." : userRole === "superadmin" ? "Create Team" : "Submit for Approval"}
              </button>
            </div>
          </div>
        </div>
