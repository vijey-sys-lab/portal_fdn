  ...
  async function fetchReviews() {
    ...
    try {
      ...
      if (userRole === "superadmin") {
        ...
        // admins and interns see their own + their team's
  ...
  async function handleSARemark() {
    ...
    try {
      await updateDoc(doc(db, "weeklyReviews", remarkModal.id), {
        saStatus: remarkStatus,
        saRemark: remarkText,
        saReviewedBy: userData.name,
        saReviewedAt: serverTimestamp(),
      });
  ...
  return (
    <Layout title="Weekly Review" subtitle="Weekly knowledge and task review">
      <div className="page-header">
        <div>
          <h2>Weekly Review</h2>
          <p>Submit your weekly learnings, tasks done, and self-assessment</p>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {(userRole === "intern" || userRole === "admin") && (
            <button className="btn btn-primary" onClick={() => setShowModal(true)}>
              <MdAdd /> Submit This Week's Review
            </button>
          )}
        </div>
      </div>
      ...
      {loading ? <div className="loading-center"><div className="spinner" /></div> : reviews.length === 0 ? (
        <div className="empty-state">
          <div className="empty-state-icon"><MdRateReview /></div>
          <h3>No Reviews Yet</h3>
          <p>{userRole === "superadmin" ? "No reviews submitted for this period" : "Submit your first weekly review"}</p>
        </div>
      ) : (
        reviews.map(r => (
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--accent-glow)", border: "2px solid var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "var(--accent)", fontSize: 13 }}>
                    {r.submittedByName?.charAt(0)}
                  </div>
                  <div>
                    <div style={{ fontWeight: 700 }}>{r.submittedByName}</div>
                    <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{r.submittedByRole} · {r.week}</div>
                  </div>
                  <div style={{ display: "flex", gap: 2 }}>
                    {[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= r.selfRating ? "var(--accent)" : "var(--border)", fontSize: 16 }}>★</span>)}
                  </div>
                ...
                <div style={{ background: "var(--bg-primary)", borderRadius: 8, padding: "10px 14px", marginBottom: 8, borderLeft: "3px solid var(--info)" }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 4 }}>Tasks Done This Week</div>
                ...
                {r.challenges && (
                  <div style={{ background: "var(--bg-primary)", borderRadius: 8, padding: "10px 14px", marginBottom: 8, borderLeft: "3px solid var(--warning)" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 4 }}>Challenges Faced</div>
                    <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>{r.challenges}</div>
                  </div>
                )}

                {r.saRemark && (
                  <div style={{ background: "var(--bg-primary)", borderRadius: 8, padding: "10px 14px", borderLeft: "3px solid var(--success)" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: "var(--purple)", textTransform: "uppercase", marginBottom: 4 }}>⭐ Super Admin Remark</div>
              ...
              {userRole === "superadmin" && (
                <button className="btn btn-secondary btn-sm" onClick={() => { setRemarkModal(r); setRemarkText(r.saRemark || ""); setRemarkStatus(r.saStatus || "approved"); }}>
                  <MdRateReview /> {r.saRemark ? "Edit Remark" : "Add Remark"}
                </button>
              )}
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal modal-lg">
            <div className="modal-header">
              <span className="modal-title">Submit Weekly Review</span>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea className="form-textarea" value={form.tasksDone} onChange={e => setForm({...form, tasksDone: e.target.value})} placeholder="List all tasks you completed this week..." />
              </div>
              <div className="form-group">
                <label className="form-label">Key Learnings *</label>
                <textarea className="form-textarea" value={form.learnings} onChange={e => setForm({...form, learnings: e.target.value})} placeholder="What did you learn this week? Skills gained, concepts understood..." />
              </div>
              <div className="form-group">
                <label className="form-label">Challenges Faced</label>
                <textarea className="form-textarea" style={{ minHeight: 80 }} value={form.challenges} onChange={e => setForm({...form, challenges: e.target.value})} placeholder="Any blockers or challenges?" />
              </div>
              <div className="form-group">
                <label className="form-label">Self Rating for This Week</label>
                <div style={{ display: "flex", gap: 8 }}>
                  {[1,2,3,4,5].map(s => (
                    <button key={s} type="button" onClick={() => setForm({...form, selfRating: s})} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 32, color: s <= form.selfRating ? "var(--accent)" : "var(--border)", transition: "color 0.2s" }}>★</button>
                  ))}
                </div>
              </div>
            </div>
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleSubmit} disabled={saving}>
                {saving ? <><div className="spinner" style={{width:16,height:16,borderWidth:2}} /> Submitting...</> : "Submit Weekly Review"}
              </button>
            </div>
          </div>
        </div>
      ...
      {remarkModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setRemarkModal(null)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontWeight: 700 }}>{remarkModal.submittedByName}</div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{remarkModal.week} · {remarkModal.submittedByRole}</div>
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <textarea className="form-textarea" value={remarkText} onChange={e => setRemarkText(e.target.value)} placeholder="Your feedback..." />
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => setRemarkModal(null)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleSARemark} disabled={saving}>
                {saving ? "Saving..." : "Submit Remark"}
              </button>
            </div>
          </div>
        </div>
