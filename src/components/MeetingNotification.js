          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 13 }}>Meeting Scheduled</span>
              </div>
              ...
              {userRole === "admin" && (
                <p style={{ fontSize: 11, color: "var(--warning)", marginBottom: 8 }}>
                  ⚠️ Please inform your interns to join on time.
                </p>
              )}
            ...
            <button onClick={() => setDismissed(true)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)" }}>
              <MdClose />
            </button>
          </div>
        </div>
        ...
        <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--warning)", borderLeft: "4px solid var(--warning)", borderRadius: 12, padding: "14px 16px", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 13, color: "var(--warning)", fontWeight: 600 }}>
              📋 {requests.length} meeting request(s) pending
            </span>
            <button onClick={() => setDismissed(true)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)" }}>
              <MdClose />
            </button>
